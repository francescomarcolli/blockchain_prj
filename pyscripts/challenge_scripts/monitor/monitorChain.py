    ...
    for log in logs:
        for log_entry in log:
            if (log_entry['event'] == "Transfer"):
                return log_entry['args']['value'] >= 1000 * 10 ** 18 # let's consider transactions of more than 1000 full tokens as suspicious (assuming that our token has 18 decimal places)
    return False
...
def handle_event(event_data, contract):
    ...
    events = [event['name'] for event in contract.events._events] #workaround for contract.events lacking an iterator, get the list of events
    logs = [ contract.events.__dict__[event_name]().processReceipt(receipt) for event_name in events ] # loop through contract.events attributes at this point you can add any conditions to log only suspicious transactions (e.g. the withdrawn value was too large)
